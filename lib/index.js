...
function requireSessionQuery(ctx) {
	...
	if (service === void 0) throw new Error("session-share: the sessionQuery service is not mounted");
	return service;
}
/**
...
*/
function parseShareInvocation(raw) {
	...
	for (let index = 0; index < tokens.length; index += 1) {
		...
		if (token === "txt") txt = true;
		else if (token === "last") {
			const count = Number(tokens[index + 1]);
			if (!Number.isInteger(count) || count <= 0) return {
				kind: "error",
				text: "/share last requires a positive message count, e.g. `/share last 10`."
			};
			lastN = count;
			index += 1;
		} else return {
			kind: "error",
			text: "/share accepts: nothing (opens the dialog), `txt` (save the whole chat as .txt), `last <n>` (save the newest n messages as .txt), or `txt last <n>`."
		};
	}
	const count = lastN === void 0 ? "" : String(lastN);
	return txt || lastN !== void 0 ? {
		kind: "success",
		text: `share:txt${count === "" ? "" : `:${count}`}`
	} : {
		kind: "success",
		text: "share"
	};
...
function toolText(event) {
	...
	const arguments_ = rawArguments.length > TOOL_ARGS_MAX_CHARS ? `${rawArguments.slice(0, TOOL_ARGS_MAX_CHARS)}…` : rawArguments;
...
*/
function shareMessagesFromEvents(events) {
	const messages = [];
	for (const event of events) {
		...
		} else if (event.type === "tool/call") messages.push({
			seq,
			role: "tool",
			time,
			text: toolText(event),
...
async function inlineImages(ctx, messages, signal) {
	...
	if (attachments === void 0 || referenced === 0) return messages.map((message) => ({
		...message,
		images: []
	}));
...
async function readSessionMessages(ctx, sessionId, includeSubagents, signal) {
	const observation = await requireSessionQuery(ctx).observeSession(sessionId, {
		signal,
		projectionMode: "none"
	});
	try {
...
async function readChildMessages(ctx, sessionId, signal) {
	...
	if (subagents === void 0 || sessionQuery === void 0) return [];
	let entries;
	try {
	...
	for (const entry of entries) {
		if (entry.kind !== void 0 && entry.kind !== "child") continue;
		...
		try {
			const observation = await sessionQuery.observeSession(childSessionId, {
				signal,
				projectionMode: "none"
			});
			try {
				for (const message of shareMessagesFromEvents(observation.events)) messages.push({
					...message,
					child: {
...
*/
async function shareRouteResponse(ctx, config, request) {
	...
	try {
		...
		const withImages = config.includeImages === false ? read.messages.map((message) => ({
			...message,
			images: []
...
function hostRenderTxt(events) {
	const lines = ["Shared from DeepSeek Harness", ""];
	for (const message of shareMessagesFromEvents(events)) lines.push(`${TXT_ROLE[message.role]} · ${new Date(message.time).toLocaleString(void 0, {
		dateStyle: "medium",
		timeStyle: "medium"
	})}`, "", message.text, "");
	return lines.join("\n").trimEnd() + "\n";
...
function apply(ctx, config = {}) {
	...
	if (connection !== void 0) ctx.effect(() => connection.fetch.register({
		path: SHARE_ROUTE,
		methods: ["GET"],
		requestBody: "buffered",
		fetch: (request) => shareRouteResponse(ctx, config, request)
	}), "session-share: share route");
	if (config.autoSaveDir !== void 0 && config.autoSaveDir.trim() !== "") {
		const dir = config.autoSaveDir.trim();
		ctx.on("session/event", (session, event) => {
			if (event.type !== "turn/end") return;
			autoSaveSession(ctx, dir, String(session.id));
		});
	}
}
/**
...
//#endregion
export { SHARE_ROUTE, apply, hostRenderTxt, inject, name, parseShareInvocation, shareMessagesFromEvents, shareRouteResponse };
