window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/row-menu.tsx
		/**
		...
		*/
		function chatShareRowMenuActions(open, saveTxt, labels) {
			return [{
				id: "chat-share",
				order: 10,
				label: () => labels().share,
		...
		function apply(ctx) {
			...
			ctx.inject(["sessionRowMenu"], (menuCtx) => {
				...
				menuCtx.effect(() => {
					...
					const disposers = chatShareRowMenuActions((sessionId) => controller.open(sessionId), (sessionId) => controller.saveTxt(sessionId), menuLabelsOf(t)).filter((action) => !registered.has(action.id)).map((action) => menuCtx.sessionRowMenu.register(action));
					return () => {
